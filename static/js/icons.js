 ...
 * -----------------------------------------------------------
 * Remplace les emojis utilisés comme icônes système (nav, boutons,
 * filtres). Trait 1.5px, grille 24×24, stroke: currentColor.
 *
 * Usage HTML :
 *   <i data-icon="phone"></i>
 ...
 *   renderIcons(myContainer);     // balaie un sous-arbre
 *
 * Les icônes injectées portent aria-hidden="true" — toujours mettre
 * un aria-label sur le bouton/lien parent.
 */
...
/**
 * Rend une icône sous forme de string SVG.
 * @param {string} name - clé dans ICON_PATHS
...
/**
 * Balaie un sous-arbre DOM et remplace chaque <i data-icon="X"> par
 * le SVG correspondant. À rappeler après toute injection dynamique.
...
function renderIcons(root) {
  ...
  root.querySelectorAll('i[data-icon]').forEach(function (el) {
    ...
    el.outerHTML = icon(name, { size, cls });
  });
}

// Auto-run au chargement du DOM
