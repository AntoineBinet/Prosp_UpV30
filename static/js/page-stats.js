...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Tabs: Statistiques / Rapport & KPI
    try {
        const tabs = Array.from(document.querySelectorAll('.stats-tab-btn'));
        const sections = {
            'stats-main': document.getElementById('statsTabMain'),
            'stats-report': document.getElementById('statsTabReport'),
        };
        ...
        tabs.forEach(btn => {
            btn.addEventListener('click', () => activateTab(btn.dataset.tab || 'stats-main'));
        });
        // Default tab
        activateTab('stats-main');
    } catch (e) {
        console.warn('Stats tabs init error', e);
    }
...
function downloadWeeklyExport() {
    let weekStr = '';
    const input = document.getElementById('reportWeekInput');
    if (input && input.value) {
        weekStr = String(input.value);
    } else {
        weekStr = _currentISOWeek();
        if (input) input.value = weekStr;
    }
    const useOllama = !!document.getElementById('reportUseOllama')?.checked;

    let url = `/api/stats/export_weekly_xlsx?week=${encodeURIComponent(weekStr)}`;
    if (useOllama) url += '&ollama=1';
...
(function() {
    ...
    setTimeout(() => {
        ...
        // Initialize week input with current week
...
// ════════════════════════════════════════════════════════════════
// KPI checklist intégrée dans l'onglet Rapport (ex-page-kpi.js)
// ════════════════════════════════════════════════════════════════
...
(function () {
  ...
  async function exportExcel() {
    const date = ($('kpiDate')?.value || '').trim() || (typeof todayISO === 'function' ? todayISO() : '');
