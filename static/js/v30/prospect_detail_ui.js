    ...
    document.addEventListener('click', function (e) {
      ...
      if (e.target.closest('.v30-fp-ev__expand, .v30-fp-ev__edit-form, .v30-fp-ev__actions, button, a, textarea, input, select')) {
        ...
        if (evEl && evEl.dataset.v30EvIdx != null) {
          ...
          // Navigation directe pour CR : ouvre la vue présentation du compte-rendu
          if ((ev.type || '') === 'cr' && ev.id) {
            if (typeof openCRViewModal === 'function') openCRViewModal(Number(ev.id));
            return;
          }
      ...
      if (openCRBtn) {
        ...
        if (crId && typeof openCRViewModal === 'function') openCRViewModal(crId);
        return;
      }
